...
export class UserController {
  ...
  public login = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // console.log("[controller] serviceResponse : ", serviceResponse);
      ...
      res.cookie(
        ...
        serviceResponse.session.session_id,
      ...
      console.error('[CONTROLLER] UNKNOWN_ERROR' + error);
  ...
  public logout = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!sid) {
        console.log('[controller] sid not found, already logged out' + sid); // 테스트로그
        return res.status(200).send();
      }

      // 서비스 레이어 호출해서 세션 정리
      await this.userService.logout(sid);
      console.log('[controller] logout success!');
      // 응답
      return res.status(200).send();
    } catch (error) {
      next(error);
  ...
  public checkSession = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const check = await this.userService.checkSessionBySid(sid);
