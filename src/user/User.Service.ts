...
export class UserService implements UserServiceInterface {
  ...
  /* userService.login (return : <LoginResult>) */
  public async login(userInputData: LoginDto): Promise<LoginResult> {
    try {
      ...
      await this.userRepository.updateSession(user.id, sid, expiresAt); // 레포지토리 -> 세션갱신
  ...
  public async logout(sid: string): Promise<void> {
    ...
    await this.userRepository.clearSession(sid); //로그아웃시 세션청소
  }

  /* 쿠키에 담긴 세션ID로 세션정보 확인하기 (return : <SessionCheckResult>) */
  public async checkSessionBySid(sid: string): Promise<SessionCheckResult> {
    const user = await this.userRepository.findUserBySessionId(sid);
